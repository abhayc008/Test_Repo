...
public class OrderDaoImpl implements OrderDao
{
    ...
    String place = "insert into order_012171(customer_email_id,total_bill,order_date) values(?,?,?)";
    
	String showOrderCust ="select * from order_012171 where customer_email_id = ?";
    
    String showAllOrder = "select * from order_012171 ";
	...
	public Order placeOrder(String customerEmailId)
	{
		try
		{
			ps = con.prepareStatement("select sum(c.medicine_qty*m.medicine_price) as TotalBill from medicine_012171 m inner join cart_012171 c on m.medicine_id = c.medicine_id where customer_email_id=?");
			...
			if(row>0) 
			{
			    ...
			    ps = con.prepareStatement("select * from order_012171 where customer_email_id = ? and order_date= ?");
			    ps.setString(1, customerEmailId);
	...
	public boolean cancelOrder(int orderId)
	{
		try 
		{
			ps = con.prepareStatement("update order_012171 set order_status =? where order_id=?");
